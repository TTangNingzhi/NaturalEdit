...
import { FONT_SIZE, BORDER_RADIUS } from "../styles/constants";
...
const CodeBlockWithMapping: React.FC<CodeBlockWithMappingProps> = ({
      ...
      try {
        ...
        if (loc === -1 && pattern.length <= BITAP_LIMIT) {
          try {
            ...
            console.error("[CodeBlockWithMapping] diff-match-patch error:", e, {
              codeSnippet: snippet,
              code,
            });
          }
        ...
        if (loc === -1 && pattern.length > BITAP_LIMIT) {
          ...
          for (let i = 0; i <= codeText.length - BITAP_LIMIT; i++) {
            ...
            try {
              ...
              score = 0;
              console.error(
                "[CodeBlockWithMapping] diff-match-patch error:",
                e,
  ...
  function getHighlightsForLine(
    ...
    const lineStartChar = codeLines
      .slice(0, lineIdx)
      .reduce((acc, l) => acc + l.length + 1, 0); // +1 for \n
    ...
    >
      {codeLines.map((line, idx) => {
        ...
        highlights.forEach((hl, i) => {
          ...
          // Color mapping removed as per user request
          segments.push(
            <span
              key={`hl-${i}`}
              style={{
        ...
        if (lastCol < line.length) {
          segments.push(<span key="plain-end">{line.slice(lastCol)}</span>);
        }
