...
async function handleHighlightCodeMapping(message: any) {
  ...
  const { selectedCode, codeSnippets, colorIndex, filename, fullPath } =
    message;

  if (
    typeof selectedCode !== "string" ||
    !Array.isArray(codeSnippets) ||
    typeof colorIndex !== "number"
  ) {
    console.warn(
      "[highlightCodeMapping] Invalid selectedCode, codeSnippets, or colorIndex."
    );
    return;
  }
  ...
  if (regionMatch.location === -1) {
    console.warn(
      "[highlightCodeMapping] Could not find selectedCode region in file."
    );
    return;
  }
  ...
  for (const snippet of codeSnippets) {
    ...
    if (match.location !== -1) {
      ...
      console.warn(
        "[highlightCodeMapping] Could not match codeSnippet in selectedCode region:",
  ...
  if (allRanges.length > 0) {
    ...
    const color =
      SUMMARY_CODE_MAPPING_COLORS[
        colorIndex % SUMMARY_CODE_MAPPING_COLORS.length
      ] + "80";
...
async function handleGetSummary(
  ...
  try {
    ...
    const conciseMappings = await buildSummaryMapping(
      selectedText,
      summary.concise
    );
    ...
    const bulletsMappings = await buildSummaryMapping(
      selectedText,
